...
# ===== CT013-2 ====
# ==================
def test_rf013_cancelar_exclusao_material_extra():
    with sync_playwright() as playwright:  # Definindo o contexto Playwright
        ...
        ############################################################
        ########## GARANTINDO MATERIAL PARA CANCELAMENTO ###########
        ############################################################
        materiais = page.locator(".MuiListItem-root").filter(
            has_text=TITULO_MATERIAL_EXTRA
        )
        quantidade_mesmos_materiais_antes = materiais.count()
        print(f"\n\nQuantidade materiais antes: {quantidade_mesmos_materiais_antes}")
        if quantidade_mesmos_materiais_antes == 0:
        ...
        ############################################################
        ########## ABRINDO O MODAL E CANCELANDO A EXCLUSÃO #########
        ############################################################
        primeiro_material = materiais.first
        expect(primeiro_material).to_be_visible()
        ...
        ############################################################
        ########## VALIDANDO QUE NADA FOI EXCLUÍDO #################
        ############################################################
        page.wait_for_timeout(TEMPO_DE_PAUSA)
        ...
        assert quantidade_mesmos_materiais_depois == quantidade_mesmos_materiais_antes
